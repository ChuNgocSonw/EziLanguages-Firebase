...
function QuestionCard({ q, index, onAction, actionType, isFromAI }: { q: QuizQuestion, index: number, onAction: (q: QuizQuestion, index: number) => void, actionType: 'add' | 'remove', isFromAI?: boolean }) {
    return (
...
export default function NewAssignmentPage() {
  ...
  const handleAddQuestionToSelection = (question: QuizQuestion, index: number) => {
    ...
    append({ ...question, isFromAI: true } as any);
  };
  ...
  const handleAddManualQuestion = (question: QuizQuestion) => {
    ...
    append({ ...question, isFromAI: false } as any);
  ...
  const handleSaveAssignment = async (data: QuestionsFormData) => {
    ...
    setIsSaving(true);
    try {
      const questionsToSave = data.questions.map(({ isFromAI, ...rest }: any) => rest);
      await createAssignment({ title: assignmentDetails.title, language: assignmentDetails.language, questions: questionsToSave });
                ...
                <div>
                    ...
                    <div className="p-4 border rounded-md min-h-[10rem] h-96 overflow-y-auto space-y-3">
                        ...
                        <Form {...selectionForm}><form onSubmit={selectionForm.handleSubmit(handleSaveAssignment)}>
                            {fields.map((field, index) => (<QuestionCard key={field.id} q={field as QuizQuestion & { isFromAI?: boolean }} index={index} onAction={handleRemoveQuestionFromSelection} actionType="remove" isFromAI={(field as any).isFromAI} />))}
                        </form></Form>
                    </div>
                </div>
            </div>
