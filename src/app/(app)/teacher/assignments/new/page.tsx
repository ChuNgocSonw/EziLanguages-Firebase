...
export default function NewAssignmentPage() {
  ...
  const handleGenerateQuestions = async (data: GenerationFormData) => {
    ...
    try {
      ...
      setGeneratedQuestions(generated.filter(gq => !fields.some(sq => sq.question === gq.question)));
    } catch (error: any) {
                ...
                <div>
                    ...
                    <div className="p-4 border rounded-md h-96 overflow-y-auto space-y-3">
                        ...
                        <Form {...selectionForm}><form onSubmit={selectionForm.handleSubmit(handleSaveAssignment)}>
                            {fields.map((field, index) => (<QuestionCard key={field.id} q={field as QuizQuestion} index={index} onAction={(_, idx) => remove(idx)} actionType="remove" />))}
                        </form></Form>
                    </div>
                </div>
            </div>
