            ...
            <div className="space-y-4 max-h-80 overflow-y-auto pr-2">
                {selectedQuiz.questions.map((q, i) => (
                    <div key={i} className="p-3 border rounded-md bg-muted/50">
                        ...
                        <p className={cn("text-sm flex items-center gap-2 mt-2", selectedQuiz.selectedAnswers[i] === q.answer ? "text-green-500" : "text-red-500")}>
                         ...
                         {selectedQuiz.selectedAnswers[i] !== q.answer && <p className="text-sm text-green-500 ml-6">Correct answer: {q.answer}</p>}
                    </div>
                ))}
            </div>
        </CardContent>
